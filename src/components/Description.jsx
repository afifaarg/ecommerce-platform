...
const Description = ({product}) => {
  ...
  return (
    <div className="w-1/2 max-lg:w-4/5">
      <h2 className="text-orange font-bold text-sm">{product.name}</h2>
      ...
      <div className="flex items-center gap-5 max-lg:flex-col max-lg:items-start max-sm:clear-right">
        ...
        <button
          // onClick={addToChart}
          className="hover:opacity-70 flex items-center justify-center gap-4 bg-primary w-60 py-3 rounded-lg max-sm:w-full"
        >
          ...
          <span className="text-white font-bold">Ajouter Au Panier</span>
        </button>
      </div>
    </div>
  );
